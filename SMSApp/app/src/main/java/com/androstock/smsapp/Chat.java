...
public class Chat extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        try {
            databse = userRoomDatabse.getDatabase(getApplicationContext());
            mUser = userViewModel.getAllUser(databse);
        } catch (Exception e) {
            e.printStackTrace();
        }
        ...
        for (int i = 0; i < mUser.size(); i++) {
            String threadIdMain=thread_id_main+"";
            if (address.equals(mUser.get(i).address )) {
                flag = true;
            }
        }
        ...
        if (!flag) {
            ...
            if (tv.getVisibility() == View.INVISIBLE) {
             tv.setVisibility(View.VISIBLE);   // do something.
        ...
        encrypt.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View view) {
                if (encrypt.getText().toString().equals("encrypt")) {
                    Intent i = new Intent(Chat.this, PasswordEntity.class);
                    i.putExtra("name", name);
                    i.putExtra("address", address);
                    i.putExtra("thread_id", thread_id_main + "");
                    //Toast.makeText(getApplicationContext(),thread_id_main+"",Toast.LENGTH_LONG).show();
                    startActivity(i);
                } else {
                    Intent i = new Intent(Chat.this, DecryptActivity.class);
                    i.putExtra("name", name);
                    i.putExtra("address", address);
                    i.putExtra("thread_id", thread_id_main + "");
                    startActivity(i);
